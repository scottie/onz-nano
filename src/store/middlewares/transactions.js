...
const transactionAdded = (store, action) => {
  const texts = {
    ...
    [transactionTypes.send]: i18next.t('Your transaction of {{amount}} ONZ to {{recipientAddress}} was accepted and will be processed in a few seconds.',
      { amount: fromRawOnz(action.data.amount), recipientAddress: action.data.recipientId }),
  };
