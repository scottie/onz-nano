...
describe('MenuBuilder', () => {
  ...
  beforeEach(() => {
    electron = {
      ...
      app: { getName: () => ('Onz Nano'), getVersion: () => ('some version') },
    };
  });
  ...
  it('Builds the electron about menu when os is mac', () => {
    ...
    expect(template[0].label).to.equal('Onz Nano');
  ...
  it('Builds the electron about menu when os is not mac', () => {
    ...
    expect(template[0].label).to.not.equal('Onz Nano');
  });

  it('Should open link on click', () => {
    menu.onClickLink(electron, 'https://onzcoin.com');
    expect(electron.shell.openExternal).to.have.been.calledWith('https://onzcoin.com');
  });
});
